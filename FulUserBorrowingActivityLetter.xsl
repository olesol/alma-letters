    ...
    <div class="messageArea">
     <div class="messageBody">
      <table border="0" cellpadding="5" cellspacing="0">
       ...
       <xsl:choose>
        <xsl:when test="(notification_data/receivers/receiver/user/user_group ='4') and (notification_data/receivers/receiver/preferred_language='no')">
         <tr>
          <td>Nedenfor følger en oversikt over dine lån. Ved forfallsdato vil bøkene bli fornyet dersom det ikke er venteliste på dem.</td>
         </tr>
        </xsl:when>

        <xsl:when test="(notification_data/receivers/receiver/user/user_group !='4') and (notification_data/receivers/receiver/preferred_language ='no')"> 
         <tr>
          <td>Nedenfor følger en oversikt over dine lån og eventuelle gebyrer. Ved forfallsdato vil bøkene bli fornyet dersom det ikke er venteliste på dem. Lån vil ikke bli forlenget om du har utestående gebyrer.</td>
         </tr>
        </xsl:when> 

        <xsl:when test="(notification_data/receivers/receiver/user/user_group ='4') and (notification_data/receivers/receiver/preferred_language !='no')"> 
         <tr>
          <td>Here is an overview of your loans at the BI Library. Your loans will be automatically renewed if there is no waiting list.</td>
         </tr>
        </xsl:when> 

        <xsl:when test="(notification_data/receivers/receiver/user/user_group !='4') and (notification_data/receivers/receiver/preferred_language !='no')"> 
         <tr>
          <td>Here is an overview of your loans (and fines, if you have any) at the BI Library. Your loans will be automatically renewed if there is no waiting list. Loans will not be renewed if you have unpaid fines.</td>
         </tr>
        </xsl:when> 
       ...
       <xsl:if test="notification_data/item_loans/item_loan or notification_data/overdue_item_loans/item_loan">
        <!-- Fjernet kobling til @@reminder_message@@ i kodetabell -->
        <!-- <tr>
         <td>@@reminder_message@@
     ...
     <tr><td><xsl:value-of select="$runningSum"/></td></tr><tr><td><xsl:value-of select="notification_data/total_fee"/></td></tr>-->
       ...
       <tr>
         ...
         <xsl:when test="notification_data/receivers/receiver/preferred_language='no'">
          <tr>
           <td>Betal gebyret i skranken i biblioteket, med 
            <xsl:choose>
             <xsl:when test="notification_data/receivers/receiver/user/campus_code = 'BIOSL'">kort.</xsl:when>
         ...
         <xsl:otherwise>
          <tr>
           <td>Please pay the fine at the library information desk with 
            <xsl:choose>
             <xsl:when test="notification_data/receivers/receiver/user/campus_code = 'BIOSL'">credit card.</xsl:when>
        ...
        <xsl:choose>
         <xsl:when test="notification_data/receivers/receiver/preferred_language='no'">
          <tr>
           <td>Vi minner om at biblioteket ilegger daglig purregebyr etter at lånet har forfalt (kr 5,- pr. bok pr. dag på vanlige lån / kr 50,- pr. bok pr dag på dagslån). Ved tap av lånt bok, krever vi erstatning for denne.</td>
          </tr>
         </xsl:when>
         <xsl:otherwise>
          <tr>
           <td>We would like to remind you about our policy: If you return books after the due date, you receive a penalty fine of 5 NOK per day for regular loans and 50 NOK per day for one day loans, for each book. If you lose the book, you have to pay for it.</td>
          </tr>
         </xsl:otherwise>
        </xsl:choose>
       </xsl:if>
